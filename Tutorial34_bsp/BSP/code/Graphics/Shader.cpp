...
/*
 ================================
 Shader::GetFileContent
 ================================
 */
bool Shader::GetFileContent( const char * file, String & str ) {
	char * source = NULL;

	unsigned int size( 0 );
	GetFileData( file, (unsigned char **)&source, size );
    if ( !source ) {
		printf( "No Shader file for:  %s\n", file );
        return false;
    }
...
/*
 ...
 ================================
 */
String Shader::AddIncludes( const String & source, const String & fileName ) {
	const String filePath = fileName.GetFilePath();

	String srcCopy = source;
	while ( 1 ) {
		const int idx = srcCopy.FindFirstStr( "#include" );
		if ( -1 == idx ) {
			break;
		}

		String leading = String( srcCopy.cstr(), idx );
		String trailing = String( srcCopy.cstr() + idx );

		const int idx2 = trailing.FindFirstStr( "\"" );
		if ( -1 == idx2 ) {
			printf( "Parsing Error... couldn't find opening quote for include\n" );
			assert( false );
			break;
		}

		String trailing2 = String( trailing.cstr() + idx2 + 1 );
		
		const int idx3 = trailing2.FindFirstStr( "\"" );
		if ( -1 == idx3 ) {
			printf( "Parsing Error... couldn't find closing quote for include\n" );
			assert( false );
			break;
		}

		String includeName = String( trailing2.cstr(), idx3 );
		String fullPath = filePath + includeName;

		String includeData;
		if ( !GetFileContent( fullPath.cstr(), includeData ) ) {
			assert( false );
			break;
		}
...
bool Shader::CompileShaderFromFile( GLuint * shader, const GLenum type, const char * file ) {
	...
	{
		String tmp;
		if ( !GetFileContent( file, tmp ) ) {
			return false;
		}

		source = AddIncludes( tmp, file );
	}
    ...
    *shader = glCreateShader( type );
	glShaderSource( *shader, 1, (const GLchar **)&source_ptr, NULL );
