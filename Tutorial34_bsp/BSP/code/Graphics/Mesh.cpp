...
bool Mesh::Load( const char * name ) {
	...
	Array< Vec3d > mPositions( 4096 );
	Array< Vec3d > mNormals( 4096 );
	Array< Vec2d > mST( 4096 );
	...
	while ( !feof( fp ) ) {
		...
		if ( sscanf( buff, "v %f %f %f", &point.x, &point.y, &point.z ) == 3 ) {
			...
			for ( int it = 0; it < 4; ++it ) {
				...
				Vec3dToByte4_n11( vert[ it ].norm, mNormals[ k[ it ] - 1 ] );
			...
			norm.Normalize();
			for ( int it = 0; it < 4; ++it ) {
				Vec3dToByte4_n11( vert[ it ].norm, norm );
			}
...
// 		v1.tang = tangent;
// 		v2.tang = tangent;
		Vec3dToByte4_n11( v0.tang, tangent );
		Vec3dToByte4_n11( v1.tang, tangent );
		Vec3dToByte4_n11( v2.tang, tangent );
	}
}
