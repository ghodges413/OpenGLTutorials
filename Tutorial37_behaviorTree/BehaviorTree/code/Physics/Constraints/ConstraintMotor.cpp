...
//  ConstraintMotor.cpp
//
#include "Physics/Constraints/ConstraintMotor.h"
...
#define BAUMGARTE_STABILIZATION
	...
	const Quat qrB = q0_inv * qr;	// Relative orientation in BodyB's space

	const Vec3 axisA = m_bodyA->m_orientation.RotatePoint( qrA.xyz() );	// World space axis for the relative rotation
	const Vec3 axisB = m_bodyB->m_orientation.RotatePoint( qrB.xyz() );	// World space axis for the relative rotation
...
void ConstraintMoverSimple::PreSolve( const float dt_sec ) {
	m_time += dt_sec;
	m_bodyA->m_linearVelocity[ axis ] = cosf( m_time * 0.25f ) * 4.0f;
}