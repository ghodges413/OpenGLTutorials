...
/*
 ...
 ===============================
 */
Sphere::Sphere( const Vec3 * pts, const int num_pts ) {
    assert( num_pts > 0 );
    mRadius = 0;
    mCenter = pts[0];
    mInitialized = true;
 ...
 * EPOS
 ===============================
 */
void Sphere::Inflate( const Vec3& pt ) {
    if ( !mInitialized ) {
        mRadius = 0;
        mCenter = pt;
        mInitialized = true;
...
/*
 ...
 ===============================
 */
void Sphere::Inflate( const Vec3 * pts, const int& num_pts ) {
 ...
 */
#include <stdio.h>
void Sphere::TightestFit( const Vec3 * pts, const int& num_pts ) {
    assert( num_pts > 2 );
...
/*
 ...
 ===============================
 */
void Sphere::GeometricFit( const Vec3 *pts, const int &num_pts ) {
    assert( num_pts > 1 );
...
/*
 ...
 ===============================
 */
void TessellateTriangle( const Vec3 & a, const Vec3 & b, const Vec3 & c, Array< Vec3 > & out ) {
//    out.Expand( 12 );
    out.Clear();
    Vec3 midAB = ( a + b ) * 0.5f;
    Vec3 midBC = ( b + c ) * 0.5f;
...
/*
 ...
 ===============================
 */
void TessellateAndNormalize( const Array< Vec3 > & in, Array< Vec3 > & out ) {
//    out.Resize( in.Num() * 4 );
    out.Clear();
...
/*
 ...
 ===============================
 */
void Sphere::BuildHalfSphereGeo( Array< Vec3 > & out, const int & numSteps ) {
...
/*
 ...
 ===============================
 */
void Sphere::BuildGeometry( Array< Vec3 > & out, const int & numSteps ) {
...
/*
 ...
 ===============================
 */
Sphere Sphere::Expand( const Vec3 & poleA, const Vec3 & poleB, const Vec3 & ext ) {
    const Vec3 center = ( poleA + poleB ) * 0.5f;
...
/*
 ...
 ================================
 */
bool Sphere::IsPointInSphere( const Vec3 & point ) const {
	if ( false == mInitialized ) {
		return false;
	}

	const Vec3 diff	= point - mCenter;
	const float distSqr	= diff.Dot( diff );
...
bool Sphere::IntersectSphere( const Sphere & rhs ) const {
	...
	const Vec3 diff	= mCenter - rhs.mCenter;
	const float distSqr	= diff.Dot( diff );
