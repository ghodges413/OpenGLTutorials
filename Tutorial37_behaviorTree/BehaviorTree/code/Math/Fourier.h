...
#define HB_PI 3.1415926535897932384626433832795f
...
public:
	...
	static void FractionalDFT( float alpha, const Complex * data_in, const int num, Complex * data_out );
	static void FractionalDFT_2D( float alpha, const Complex * data_in, const int dim[ 2 ], Complex * data_out );
	...
	// very special bullshit for helping us get fourier on gpu working
...
inline Complex Fourier::W( const int nk, const int N ) {
	const float angle = -2.0f * HB_PI * (float)nk / (float)N;
...
inline Complex Fourier::W_2D( const int nk[ 2 ], const int N[ 2 ] ) {
	const float angle = -2.0f * HB_PI * ( (float)nk[ 0 ] / (float)N[ 0 ] + (float)nk[ 1 ] / (float)N[ 1 ] );
...
inline Complex Fourier::W_3D( const int nk[ 3 ], const int N[ 3 ] ) {
	const float angle = -2.0f * HB_PI * ( (float)nk[ 0 ] / (float)N[ 0 ] + (float)nk[ 1 ] / (float)N[ 1 ] + (float)nk[ 2 ] / (float)N[ 2 ] );
