...
//uniform sampler2D s_shadow;	// shadow from the sun's perspective

//in vec2 coords;
//in vec3 v_ray;
//in mat4 v_viewInverse;
...
void main() {
...
#if 1
    {
		...
		vec3 color = v_color * dot( sunDir, v_normal ) * trans2;	// TODO: Calculate how much sunlight makes it to this point via transmittance

		diffuseColor.rgb = color * trans;
		//diffuseColor.rgb = trans;
		
		float shadow = 1.0;
		diffuseColor.rgb = GroundColorGeo( x, v, sunDir, r, mu, v_normal, shadow ) * trans2 * v_color;
		...
		//float r, mu;
		//vec3 inscatterColor = Inscatter( x, v, sunDir, r, mu ); //S[L]-T(x,xs)S[l]|xs = S[L] for spherical ground
		//vec3 groundColor = GroundColor( x, v, sunDir, r, mu ); //R[L0]+R[L*]
		//vec3 sunColor = SunColor( x, v, sunDir, r, mu ); //L0

		//diffuseColor.rgb += inscatterColor;
		//diffuseColor *= 0.01;
		//diffuseColor += vec4( sunColor + groundColor + inscatterColor, 1.0 ); // Eq (16)
	}
#endif	
    //FragColor = texture(inscatterSampler,vec3(coords,(s.x+1.0)/2.0));
    //FragColor = vec4(texture(irradianceSampler,coords).rgb*5.0, 1.0);
    //FragColor = texture(transmittanceSampler,coords);
	diffuseColor.a = 1.0;
}
...
void main() {
	...
	diffuseColor.rgb = v_color.rgb * light;
	diffuseColor.a = 1.0f;
//	diffuseColor.rgb *= 0.001f;
//	diffuseColor.rgb += v_normal;
//	diffuseColor.b = 1.0f;
}
#endif
