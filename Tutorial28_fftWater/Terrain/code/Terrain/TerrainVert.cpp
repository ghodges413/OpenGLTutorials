...
Vec3d CalculateColor( Vec3d normal, Vec4d sample ) {
	...
	diffuseColor.x = clampf( diffuseColor.x, 0.0f, 1.0f );
	diffuseColor.y = clampf( diffuseColor.y, 0.0f, 1.0f );
	diffuseColor.z = clampf( diffuseColor.z, 0.0f, 1.0f );
...
Vec2d HeightmapToST( int x, int y, int tileX, int tileY ) {
	...
	if ( st.x < 0.0f || st.x > 1.0f ) {
		...
		st.x = clampf( st.x, 0.0f, 1.0f );
	}
	if ( st.y < 0.0f || st.y > 1.0f ) {
		printf( "Oh no!  ST out of range!:  %f %f\n", st.x, st.y );
		st.y = clampf( st.y, 0.0f, 1.0f );
	}
