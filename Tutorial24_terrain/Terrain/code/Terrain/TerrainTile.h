...
//#define TILE_SIZE ( 1024 + 1 )			// 2km with 2 verts/meter = 2048 * 2 = 4096
...
struct terrainlet_t {	// bad name, think of something better (terrainlet_t?)
	...
	VertexBufferObject ibo;	// this could be static and used across all of them
	VertexArrayObject vao;	// this could be static and used across all of them

	terrainVert_t verts[ TERRAINLET_SIZE * TERRAINLET_SIZE ];
	unsigned short indices[ TERRAINLET_SIZE * TERRAINLET_SIZE * 6 ];	// this could be static and used across all of them
};

struct terrainNode_t {	// One of the nodes in the quad tree
	terrainNode_t( terrainVert_t * data, int width );
	~terrainNode_t() {
...
private:
	void Build( terrainVert_t * data );	// assumes a 2k x 2k image of height map data
public:
	// 4096 = 2^12
	// 128 = 2^7
