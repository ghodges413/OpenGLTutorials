...
mPlanarPoint( point ),
mNormal( normal ) {
    mD = -mPlanarPoint.DotProduct( mNormal );
}
...
bool Plane::IntersectPlane( const Plane & rhs, Vec3d & start, Vec3d & dir ) const {
	const float absoluteDot = mNormal.DotProduct( rhs.mNormal );
...
bool Plane::IntersectRay( const Vec3d & start, const Vec3d & dir, float & t, Vec3d & p ) const {
	...
	const float cosTheta = mNormal.DotProduct( normDir );
