...
plane_t PlaneFromWinding( const winding_t & winding ) {
	...
	Vec3 b = winding.pts[ 1 ];
	Vec3 c = winding.pts[ 2 ];
...
================================
*/
int GetPointSide( Vec3 p, const plane_t * split ) {
	Vec3 planeToPoint = p - split->pts[ 0 ];
...
void WindingToVerts( const winding_t & winding, std::vector< vert_t > & verts, std::vector< int > & indices ) {
	...
	const Vec3 norm = CalculateNormal( winding.pts[ 0 ], winding.pts[ 1 ], winding.pts[ 2 ] );
	for ( int v = 0; v < winding.pts.size(); v++ ) {
		vert_t vert;
		memset( &vert, 0, sizeof( vert ) );
		vert.pos = winding.pts[ v ];

		vert.st = Vec2( vert.pos.x, vert.pos.y );
		float x2 = norm.x * norm.x;
		float y2 = norm.y * norm.y;
		float z2 = norm.z * norm.z;
		if ( x2 > y2 && x2 > z2 ) {
			vert.st = Vec2( vert.pos.y, vert.pos.z );
		}
		if ( y2 > x2 && y2 > z2 ) {
			vert.st = Vec2( vert.pos.x, vert.pos.z );
		}
		vert.st *= 0.001f;
...
void DrawBSP_r( bsp_t * node, Shader * shader ) {
	...
	for ( int i = 0; i < verts.size(); i++ ) {
		Vec3ToByte4_01( verts[ i ].buff, Vec3( 1, 1, 1 ) );
	}
...
void DrawSplitPlaneBSP_r( bsp_t * node, Shader * shader, Vec3 camPos, int depth ) {
	...
	for ( int i = 0; i < verts.size(); i++ ) {
		Vec3ToByte4_01( verts[ i ].buff, color );
	}
