...
void Fourier::Radix4_r( Complex * data, const int power, Complex * pingPong ) {
	...
	for ( int n = 0; n < quarterNum; n++ ) {
		const Complex j = Complex( Vec2d( 0, 1 ) );
...
void Fourier::Radix8_r( Complex * data, const int power, Complex * pingPong ) {
	...
	for ( int x = 0; x < 8; x++ ) {
		for ( int y = 0; y < 8; y++ ) {
			const float angle = Math::PI * 2.0f * float( x * y ) / 8.0f;
...
//	const int num = 8 * 8 * 8;
	...
	for ( int i = 0; i < num; i++ ) {
		fft[ i ] = Complex( Vec2d( 0, 0 ) );
		inv[ i ] = Complex( Vec2d( 0, 0 ) );
	}

	const int us_start = GetTimeMicroseconds();

	Fourier::Radix4( data, num, fft );
	Fourier::InverseRadix4( fft, num, inv );

	const int us_end = GetTimeMicroseconds();
...
void Fourier::Test2D() {
	...
	const int us_start_dft = GetTimeMicroseconds();

	//Fourier::DFT_2D( data, dim, fft );
	//Fourier::InverseDFT_2D( fft, dim, inv );

	const int us_end_dft = GetTimeMicroseconds();
...
//	}
	...
	const int us_start_FFT = GetTimeMicroseconds();

	Fourier::Radix8_2D( data, dim, fft );
	Fourier::InverseRadix8_2D( fft, dim, inv );

	const int us_end_FFT = GetTimeMicroseconds();
