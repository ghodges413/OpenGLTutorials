...
bool Targa::Load( const char * fileName, const bool verbose /* = false */ ) {
    ...
    if ( 32 == mBitsPerPixel ) {
        ...
        for ( int i = 0; i < finalImageSize; i += 4 ) {
            ...
            Swap( mData_ptr[ i + 0 ], mData_ptr[ i + 2 ] );
        }
        ...
        for ( int i = 0; i < numPixels; ++i ) {
            ...
            // Convert from BGRA to RGBA
            Swap( mData_ptr[ offset32 + 0 ], mData_ptr[ offset32 + 2 ] );
        }
	...
	if ( mOrdering & TARGA_ORDER_BOTTOM_TO_TOP ) {
		...
		for ( int y = 0; y < mHeight >> 1; ++y ) {
			const int farY = mHeight - y - 1;
			for ( int x = 0; x < mWidth; ++x ) {
				for ( int i = 0; i < 4; ++i ) {
					Swap( mData_ptr[ ( mWidth * y + x ) * 4 + i ], mData_ptr[ ( mWidth * farY + x ) * 4 + i ] );
				}
			}
		}
    }
