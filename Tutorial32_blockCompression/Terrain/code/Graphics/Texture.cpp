...
//		case TT_TEXTURE_CUBEMAP_ARRAY: { target = GL_TEXTURE_CUBE_MAP_ARRAY; } break;
	...
	int size = 0;	// only used by compressed images
	...
	if ( isCompressed ) {
		switch ( m_opts.type ) {
			case TT_TEXTURE_1D: { glCompressedTexImage1D( target, 0, internalFormat, m_opts.dimX, 0, size, data ); } break;
			default:
			case TT_TEXTURE_2D: { glCompressedTexImage2D( target, 0, internalFormat, m_opts.dimX, m_opts.dimY, 0, size, data ); } break;
			case TT_TEXTURE_2D_ARRAY:
			case TT_TEXTURE_3D: { glCompressedTexImage3D( target, 0, internalFormat, m_opts.dimX, m_opts.dimY, m_opts.dimZ, 0, size, data ); } break;
		}
	} else {
		switch ( m_opts.type ) {
