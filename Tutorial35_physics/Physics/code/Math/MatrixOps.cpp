...
//    0                 0                 0                 1
    ...
    Vec3 Look = Eye - At;                // The look direction
    Look.Normalize();
    Vec3 Left = Up.Cross( Look );        // The left direction
    Left.Normalize();
    Vec3 trueUp = Look.Cross( Left );    // The up direction
    trueUp.Normalize();
    
    myLook( Eye, Look, trueUp, out );
}
void myLook( const Vec3 & Eye, const Vec3 & Look, const Vec3 & Up, float * out ) {
    const Vec3 & zaxis = Look;               // The look direction
    const Vec3   xaxis = Up.Cross( Look );   // The left direction
    const Vec3 & yaxis = Up;                 // The up direction
