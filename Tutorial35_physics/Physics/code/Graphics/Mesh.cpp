...
#if 0
void Mesh::GenerateMeshFromHeightmap( const Vec4 * samples, const int dimX, const int dimY ) {
	mVerts.Clear();
	...
	for ( int y = 0; y < dimY - 1; y++ ) {
		for ( int x = 0; x < dimX - 1; x++ ) {
			...
			Vec4 sampleA = samples[ idxA ];
			Vec4 sampleB = samples[ idxB ];
			Vec4 sampleC = samples[ idxC ];
			Vec4 sampleD = samples[ idxD ];

			// TODO: scale and recenter the x/y
			Vec3 posA = Vec3( x0, y0, sampleA.x );
			Vec3 posB = Vec3( x0, y1, sampleB.x );
			Vec3 posC = Vec3( x1, y1, sampleC.x );
			Vec3 posD = Vec3( x1, y0, sampleD.x );
...
// 			if ( y == 0 && x == 0 ) {
// 				Vec3 posA = Vec3( 0, 0, 0 ) * 10.0f;
// 				Vec3 posB = Vec3( 1, 0, 0 ) * 10.0f;
// 				Vec3 posC = Vec3( 1, 1, 1 ) * 10.0f;
// 				Vec3 posD = Vec3( 0, 1, 1 ) * 10.0f;
// 			}

			// TODO: Actually do this
// 			Vec3 normA = Vec3( 0, 0, 1 );
// 			Vec3 normB = Vec3( 0, 0, 1 );
// 			Vec3 normC = Vec3( 0, 0, 1 );
// 			Vec3 normD = Vec3( 0, 0, 1 );

			Vec3 normA = CalculateNormal( x0, y0, dimX, dimY, samples );
			Vec3 normB = CalculateNormal( x0, y1, dimX, dimY, samples );
			Vec3 normC = CalculateNormal( x1, y1, dimX, dimY, samples );
			Vec3 normD = CalculateNormal( x1, y0, dimX, dimY, samples );

			Vec3 colorA = CalculateColor( posA, normA, sampleA );
			Vec3 colorB = CalculateColor( posB, normB, sampleB );
			Vec3 colorC = CalculateColor( posC, normC, sampleC );
			Vec3 colorD = CalculateColor( posD, normD, sampleD );

			vert_t vertA;
			vertA.pos	= posA;
			vertA.norm	= normA;
			vertA.tang	= colorA;
			vertA.st	= Vec2( x0, y0 ) * ( 1.0f / dimX );

			vert_t vertB;
			vertB.pos	= posB;
			vertB.norm	= normB;
			vertB.tang	= colorB;
			vertB.st	= Vec2( x0, y1 ) * ( 1.0f / dimX );

			vert_t vertC;
			vertC.pos	= posC;
			vertC.norm	= normC;
			vertC.tang	= colorC;
			vertC.st	= Vec2( x1, y1 ) * ( 1.0f / dimX );

			vert_t vertD;
			vertD.pos	= posD;
			vertD.norm	= normD;
			vertD.tang	= colorD;
			vertD.st	= Vec2( x1, y0 ) * ( 1.0f / dimX );
