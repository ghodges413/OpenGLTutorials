...
uniform mat4 matModelViewProjPrev;
...
void DoOcean( in mat4 mvp ) {
	...
	v_position	= matModelView * oceanPos;
...
void main() {
	...
	gl_Position     = matModelViewProj * vert;
    
	//  Copy the texture coordinates over
	v_texCoord      = st;

	//  Transform the position to view space
	v_position      = matModelView * vert;
    
	//  Transform the normal and tangent to view space
	vec4 norm	= vec4( normal,     0.0 );
	vec4 tang	= vec4( tangent,    0.0 );
	v_normal	= matModelView * norm;
	v_tangent	= matModelView * tang;
	
	// Calculate the screen space motion vector
	v_screenSpacePrev	= matModelViewProjPrev * vert;
	v_screenSpace			= gl_Position;
#else
	DoOcean( matModelViewProj );
#endif
}
