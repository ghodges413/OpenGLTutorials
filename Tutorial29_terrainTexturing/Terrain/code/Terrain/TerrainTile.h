...
#define TERRAIN_ISLAND_SIZE ( 128 + 1 )		// 64m with 2 verts/meter = 64 * 2 = 128
...
struct terrainIsland_t {	// bad name, think of something better (terrainIsland_t?)
	...
	vert_t verts[ TERRAIN_ISLAND_SIZE * TERRAIN_ISLAND_SIZE ];

	static VertexBufferObject ibo;	// this could be static and used across all of them
	static unsigned short indices[ TERRAIN_ISLAND_SIZE * TERRAIN_ISLAND_SIZE * 6 ];	// this could be static and used across all of them
};
...
class TerrainTile {
public:
	TerrainTile() { m_root = NULL; m_tileX = 0; m_tileY = 0; m_colorMap = NULL; }
	~TerrainTile() { delete m_root; m_root = NULL; delete m_colorMap; }

	void Build( int tileX, int tileY );
	Vec3d GetSurfacePos( Vec3d pos ) const;
	Vec4d GetColorMap( Vec3d pos ) const;
...
public:
	...
	Vec4d * m_colorMap;	// The heightmap in the height map file.  It is used for looking up material type for the virtual texture.
};